...
anychart.onDocumentReady(function () {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            for (var category in data.categoryData) {
                categoryData.push({ x: category, value: data.categoryData[category].totalAmount });
            }
    ...
    function configurePieChart(chart) {
        ...
        chart.legend().fontSize("18px");
        chart.title().fontColor("black");
        chart.title().fontSize("22px");
        chart.legend().position("bottom");
        chart.labels().fontSize("14px");
        chart.labels().position("outside");
        chart.labels().fontColor("black");
        chart.connectorStroke({color:"black",thickness:1});
    }
